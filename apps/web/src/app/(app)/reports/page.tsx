...
const fieldStyle: React.CSSProperties = {
  ...
  backgroundColor: 'var(--c-1a1d26)', border: '1px solid rgba(var(--fg-rgb),0.1)', borderRadius: 9, outline: 'none',
};
    ...
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20, maxWidth: 980 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <div style={{ marginBottom: 4 }}>
          <h1 style={{ fontSize: 18, fontWeight: 660, color: 'var(--c-f2f3f5)', letterSpacing: '-.02em', margin: '0 0 4px' }}>Reports</h1>
          <p style={{ fontSize: 12, color: 'var(--c-767b86)', margin: 0 }}>Spend breakdown across categories and tools.</p>
        </div>
        <DownloadBtn
          onClick={() => {
            if (tab === 'spend') {
      ...
      {/* Tab switcher */}
      <div style={{ display: 'flex', gap: 4, background: 'var(--c-121419)', border: '1px solid var(--c-1e212a)', borderRadius: 10, padding: 3, alignSelf: 'flex-start' }}>
      ...
      {tab === 'spend' && (
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
            {reportStats.map((s) => (
              <div key={s.label} style={{ background: 'var(--c-101218)', border: '1px solid var(--c-1e212a)', borderRadius: 13, padding: '16px 18px' }}>
                <div style={{ fontSize: 11.5, color: 'var(--c-878c96)', marginBottom: 9 }}>{s.label}</div>
                <div style={{ fontSize: 23, fontWeight: 680, color: 'var(--c-f2f3f5)', letterSpacing: '-.02em', lineHeight: 1 }}>{s.value}</div>
                <div style={{ fontSize: 11, color: 'var(--c-6b707b)', marginTop: 7 }}>{s.sub}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: 'var(--c-0e1014)', border: '1px solid var(--c-1a1d24)', borderRadius: 14, padding: '22px 24px' }}>
            <h3 style={{ fontSize: 14, fontWeight: 650, color: 'var(--c-e6e8ec)', margin: '0 0 4px' }}>Spend by Category</h3>
            <p style={{ fontSize: 12, color: 'var(--c-767b86)', margin: '0 0 20px' }}>Monthly spend &amp; pre-paid consumption grouped by category.</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {categories.length === 0 && <p style={{ fontSize: 13, color: 'var(--c-5e636e)', margin: 0 }}>No spend data for this period.</p>}
              {categories.map((c) => (
                <div key={c.category}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 7 }}>
                    <span style={{ fontSize: 12.5, fontWeight: 550, color: 'var(--c-cfd3da)' }}>{CAT_LABELS[c.category] || c.category}</span>
                    <span style={{ fontSize: 12.5, fontWeight: 650, color: 'var(--c-f2f3f5)' }}>{fmtAmt(c.total)}</span>
                  </div>
                  <div style={{ height: 9, borderRadius: 999, background: 'var(--c-16191f)', overflow: 'hidden' }}>
      ...
      {tab === 'billing' && (
        <>
          {/* Period selector */}
          <div style={{ background: 'var(--c-0e1014)', border: '1px solid var(--c-1a1d24)', borderRadius: 14, padding: 18 }}>
          ...
          {/* Billing table */}
          <div style={{ background: 'var(--c-0e1014)', border: '1px solid var(--c-1a1d24)', borderRadius: 14, overflow: 'hidden' }}>
            {/* Header */}
            <div style={{ display: 'grid', gridTemplateColumns: '1.8fr 1fr 0.9fr 0.9fr 1fr 100px', padding: '11px 20px', background: 'var(--c-0c0e12)', borderBottom: '1px solid var(--c-1a1d24)' }}>
            ...
            {filteredBilling.map((r, i) => (
              <div key={r.id} style={{ display: 'grid', gridTemplateColumns: '1.8fr 1fr 0.9fr 0.9fr 1fr 100px', alignItems: 'center', padding: '13px 20px', borderBottom: i < filteredBilling.length - 1 ? '1px solid var(--c-15181e)' : 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 550, color: 'var(--c-e6e8ec)' }}>{r.tool?.name || 'Deleted tool'}</span>
                </div>
                <div style={{ fontSize: 12, color: 'var(--c-9aa0ab)' }}>{CAT_LABELS[r.tool?.category || ''] || r.tool?.category || '-'}</div>
                <div style={{ fontSize: 12, color: 'var(--c-9aa0ab)' }}>{r.monthLabel}</div>
                <div style={{ fontSize: 12, color: 'var(--c-9aa0ab)' }}>{periodRangeLabel(period, customFrom, customTo)}</div>
                <div style={{ fontSize: 13.5, fontWeight: 650, color: 'var(--c-f2f3f5)', fontVariantNumeric: 'tabular-nums' }}>{fmtAmt(r.amount)}</div>
                <div>
                  {(() => {
                    const isLive = r.id.startsWith('live-');
                    const label = r.status === 'PAID' ? 'Paid' : isLive ? 'In progress' : 'Pending';
                    const color = r.status === 'PAID' ? 'var(--c-3fb950)' : isLive ? 'var(--c-9aa2ef)' : 'var(--c-d99e3e)';
            ...
            {filteredBilling.length === 0 && (
              <div style={{ padding: '24px 20px', textAlign: 'center', fontSize: 13, color: 'var(--c-5e636e)' }}>No billing records found</div>
            )}
            {/* Footer */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 20px', background: 'var(--c-0a0c10)', borderTop: '1px solid var(--c-1a1d24)' }}>
              <span style={{ fontSize: 12, color: 'var(--c-6b707b)' }}>{filteredBilling.length} records</span>
              <span style={{ fontSize: 13, fontWeight: 650, color: 'var(--c-f2f3f5)' }}>{fmtAmt(filteredTotal)}</span>
            </div>
          </div>
        </>
      )}
...
function DownloadBtn({ onClick, label }: { onClick: () => void; label: string }) {
  ...
  return (
    <button
      ...
      style={{ display: 'inline-flex', alignItems: 'center', gap: 7, padding: '8px 14px', borderRadius: 9, background: hover ? 'var(--c-1f2330)' : 'var(--c-171a22)', border: '1px solid var(--c-2a2e3d)', color: 'var(--c-9aa2ef)', fontSize: 12.5, fontWeight: 600, cursor: 'pointer', transition: 'all .15s', whiteSpace: 'nowrap' }}
    >
...
function TabBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} style={{ fontFamily: 'inherit', fontSize: 12.5, fontWeight: 550, padding: '7px 16px', borderRadius: 7, cursor: 'pointer', border: `1px solid ${active ? 'var(--c-2a2e3d)' : 'transparent'}`, background: active ? 'var(--c-1b1e26)' : 'transparent', color: active ? 'var(--c-e6e8ec)' : 'var(--c-6b707b)' }}>
      {children}
    </button>
  );
}
